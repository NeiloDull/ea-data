  ...
  list(
    ...
    , analyze = list(
      ...
      list(
        ...
        , "GWWC non-students donating >10% (2015)" = function(df) ctab(dplyr::filter(df, student == "No", income_2016_individual_c > 10000), p_donate_2016 >= 0.1, member_gwwc, na.rm = TRUE)
        , "GWWC non-students donating >10% (2016)" = function(df) ctab(dplyr::filter(df, student == "No", income_2016_individual_c > 10000), p_donate_2016 >= 0.1, member_gwwc, na.rm = TRUE)
        , "GWWC non-students donating >10% (2015 and 2016)" = function(df) ctab(dplyr::filter(df, student == "No", income_2016_individual_c > 10000), p_donate_2015 >= 0.1 & p_donate_2016 >= 0.1, member_gwwc, na.rm = TRUE)
        , "GWWC non-students donating >10% (2015 or 2016)" = function(df) ctab(dplyr::filter(df, student == "No", income_2016_individual_c > 10000), p_donate_2015 >= 0.1 | p_donate_2016 >= 0.1, member_gwwc, na.rm = TRUE)
        , "GWWC non-students donating >10% (2015 - 2016 average)" = function(df) ctab(dplyr::filter(df, student == "No", income_2016_individual_c > 10000), p_donate_2015 + p_donate_2016 >= 0.2, member_gwwc, na.rm = TRUE)
        , "GWWC pledge adherence by year" = function(df) tab(dplyr::filter(df, income_2016_individual_c > 10000, student == "No", member_gwwc == "Yes"), p_donate_2015 >= 0.1, p_donate_2016 >= 0.1, na.rm = TRUE, percent = TRUE)
        , "Number of orgs people donate to" = function(df) { get_vars(df, "donate_.+2016_c") %>% grep("cause", ., invert = TRUE, value = TRUE) %>% df[, .] %>% ap ply(., 1, function(x) x > 0) %>% apply(., 2, sum) %>% table }
      )
    )
  )
})
